...
from plot_scripts import plot_original_function_1D, plot_basis_functions_1D, plot_spartity_plot, plot_lp_sparsity,plot_ground_truth

def gauss_quad(M, min_, max_):
    [X, W] = p_roots(M)
    dx = max_- min_
    dxi = 2
    W = W*(dx/dxi)
...
def local_ecm(Matrixlist, vectorlist, swap_functions, constrain_sum_of_weights ):
    ...
    print('\n number of TOTAL elements: ',WeightsMatrix.shape[0])
    print('number of times unsuccesful candidate set usage: ',unsuccesfull_index)
...
def example_paper_local_ecm(number_of_fucntions, number_of_candidate_Gauss_points, funtion_to_use, swap_functions, constrain_sum_of_weights):
    n = number_of_fucntions
    ...
    [X,W] = gauss_quad(M, min_, max_) #Gauss points and weights as candidates
    ...
    plot_spartity_plot(weights_local_ecm.T, 'local_ecm')
    ...
    plot_spartity_plot(matrix, 'multiple_independent')
    ...
    plot_spartity_plot(matrix, 'global')




    # print('\n\nselected elements by linear programming: ', indexes_lp)
    # print('selected weights by linear programming: ', weights_lp)
    # print('\n\nselected elements by local ECM: ', indexes_local_ecm)
    # print('selected weights by local ECM: ', weights_local_ecm)
    # print('\n\nselected elements by multiple independent ECMs: ', index_list_independent_ecms)
    # print('selected weights multiple independent local ECM: ', weight_list_independent_ecm)
    # print('\n\nselected elements by global ECM: ', index_single_ecm)
    # print('selected weights global ECM: ', weights_single_ecm)
    ...
    # print all approximations
    print('\n\n\n\n\n\n\n\n\n\n\n')
    print('exact: ', np.squeeze(b))
    print('lp: ', np.squeeze(lp))
    print('global ecm: ', np.squeeze(global_ecm_approximation) )
    print('independent ecms: ', np.squeeze(independent_ecm_approximation))
    print('local ecm: ', np.squeeze(local_ecm_approximation))
...
if __name__=='__main__':
    ...
    number_of_fucntions = 50
    number_of_candidate_Gauss_points = 200

    funtion_to_use = 2 # 1 or 2
    swap_functions = False # True or False. This changes the order of the functions to integrate
    constrain_sum_of_weights = True

    example_paper_local_ecm(number_of_fucntions, number_of_candidate_Gauss_points, funtion_to_use, swap_functions, constrain_sum_of_weights)
